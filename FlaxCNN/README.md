...
- **Small bugs hide in the boring parts.** The bugs in this project were not in the ML: a shadowed Python builtin (input instead of inputs), a variable that only existed on one branch of an if/else, and a pickle call missing its file argument that would have crashed after 21 epochs of training. On a cluster, anything that runs after hours of compute deserves the most scrutiny, not the least.
